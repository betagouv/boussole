...
RSpec.describe HousingAge, type: :model do
  ...
  context "when #maximum is set and #minimum isn't" do
    subject(:housing_age) { build(:housing_age, maximum: 20) }
  ...
  context "when #minimum is set and #maximum isn't" do
    subject(:housing_age) { build(:housing_age, minimum: 20) }
